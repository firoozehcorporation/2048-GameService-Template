...
// Search disable PossibleMultipleEnumeration
// Search disable CheckNamespace
// Search disable ConvertClosureToMethodGroup

namespace Sourav.Utilities.Tools
{

    internal class SearchItemInfo
    {

        public SearchItemInfo(String fullPath)
        {
        ...
        public SearchItemInfo(string parentPath, string name, params Object[] unityObjects)
        {
    ...
    {
        public Func<string, IEnumerable<SearchItemInfo>> RefreshAction { get; set; }
        public Func<SearchItemInfo, Object[]> LoadItem { get; set; }
    ...
    public class SearchWindow : EditorWindow
    {
        ...
        private IEnumerable<SearchItemInfo> itemInfos = NoResults;
        ...
        private static readonly SearchItemInfo[] NoResults = { };
        ...
        [MenuItem("ProjectUtility/Tools/Search/Go To Asset %t")]
        ...
        [MenuItem("ProjectUtility/Tools/Search/Go To Game Object %g")]
        [SuppressMessage("Search", "UnusedMember.Local")]
        ...
        [MenuItem("ProjectUtility/Tools/Search/Change Window Location")]
        [SuppressMessage("Search", "UnusedMember.Local")]
        ...
        [MenuItem("ProjectUtility/Tools/Search/Clear Recent Items")]
        [SuppressMessage("Search", "UnusedMember.Local")]
        static void ClearRecentItems()
        {
            SearchSelectionHistoryTracker.PreviousSelections.Clear();   
        }

        private static IEnumerable<SearchItemInfo> RefreshRecentItemInfos(string itemName)
        {
        ...
        private static SearchWindow PreviousWindow;
        private bool darkSkin;

        private static void ShowWindow(SearchMode mode)
        {
            ...
            var search = CreateInstance<SearchWindow>();
            search.wantsMouseMove = true;
            search.mode = mode;
            if (mode.MoveWindowMode)
            {
                search.minSize = new Vector2(WINDOW_WIDTH, MOVE_WINDOW_HEIGHT);
                search.maxSize = new Vector2(WINDOW_WIDTH, MOVE_WINDOW_HEIGHT);
                search.ShowUtility();
                search.position = PositionRect(MOVE_WINDOW_HEIGHT);
            }
            else
            {                
        ...
        private static IEnumerable<SearchItemInfo> RefreshAssetInfos(string itemName)
        {
        ...
        private static IEnumerable<SearchItemInfo> RefreshInfos(string itemName, Func<string[], IEnumerable<Type>, IEnumerable<SearchItemInfo>> findAction, bool returnAllOnEmptySearch = false)
        {
            InitUnityTypes();
            if (string.IsNullOrEmpty(itemName))
            {
        ...
        private static IEnumerable<SearchItemInfo> ResultsFor(List<string> searchTerms, IEnumerable<Type> unityTypes,
            Func<string[], IEnumerable<Type>, IEnumerable<SearchItemInfo>> findAction)
        {
        ...
        private static IEnumerable<SearchItemInfo> FindAssetsAction(string[] searchTerms, IEnumerable<Type> unityTypes)
        {
            ...
            if (searchFor.Trim().Length == 0)
                return Enumerable.Empty<SearchItemInfo>();
            var reUniterItemInfos = AssetDatabase.FindAssets(searchFor, new[] { "Assets" })
                .Select(x => new SearchItemInfo(AssetDatabase.GUIDToAssetPath(x).Substring("Assets/".Length)));
            return reUniterItemInfos;
        }

        private static IEnumerable<SearchItemInfo> FindGameObjectsAction(string[] searchTerms, IEnumerable<Type> unityTypes)
        {
...
//                .SelectMany(x => Resources.FindObjectsOfTypeAll(x)).Where(x => !AssetDatabase.Contains(x) && x.name.Trim() != "") // returns all game objects, including inactive ones
                .Select(x => new SearchItemInfo(BuildParentPath(x), x.name, x)); 
        }
        ...
        private static int ComputePenalty(SearchItemInfo item, List<string> searchTerms)
        {
        ...
        [SuppressMessage("Search", "LoopCanBeConvertedToQuery")] //foreach more efficient than LINQ
        ...
        private static IEnumerable<SearchItemInfo> RefreshGameObjectInfos(string itemName)
        {
            return RefreshInfos(itemName, FindGameObjectsAction);
        }

        [SuppressMessage("Search", "CanBeReplacedWithTryCastAndCheckForNull")]
        ...
        private void DisplaySearchTextField()
        {
            ...
            EditorGUILayout.LabelField("<b><color=#85C226FF>Keeves</color><color=#F8C301FF>Search</color></b>", RightAlignRichTextGuiStyle);
            GUILayout.EndHorizontal();
...
# else
        ...
        private string Highlight(SearchItemInfo itemInfo, string searchTerms)
        {
        ...
        private void SelectItems(IEnumerable<SearchItemInfo> items, bool appendToSelection, bool openAssets, bool onlyPing)
        {
            if (onlyPing)
            {
        ...
        private static Object[] LoadAsset(SearchItemInfo itemInfo)
        {
        ...
        private static Object[] LoadUnityObjects(SearchItemInfo itemInfo)
        {
